...
namespace MSS6x_Tool
{
    ...
    {
        public static byte[] GetSecurityAccessMessage(byte[] userId, byte[] serialNumber, byte[] seed)
        {
            BigInteger n = 1;
            BigInteger d = 0;

            if (Global.HwRef == "0569Q60")
            {
            ...
            var hash = md5Hash.ComputeHash(toHash);
            ...
            byte[] authHeader = { 01, 00, 00, 00, 0x00, 00, 00, 00, 00, 00, 00, 00, 00, 0x44, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 0x10 };
            byte[] authMessage = authHeader.Concat(authPayload).ToArray();
            
            return authMessage;
        }

        public static byte[] CorrectParameterChecksum(byte[] dataToFlash)
        {
            ...
            {
                var checksumStart = BitConverter.ToUInt32(dataToFlash.Skip(0x1C4 + 8 * i).Take(4).Reverse().ToArray(), 0) - memSubtract;

                var checksumEnd = BitConverter.ToUInt32(dataToFlash.Skip(0x1C8 + 8 * i).Take(4).Reverse().ToArray(), 0) - memSubtract;
            ...
            byte[] csCalcArray = BitConverter.GetBytes(crc).ToArray();
            for (int i = 0; i < 4; ++i)
                dataToFlash[0xFFFC + i] = csCalcArray[3 - i];
        ...
        public static byte[] CorrectProgramChecksum(byte[] dataToFlash)
        {
            ...
            for (int i = 0; i < numberOfSegments; ++i)
            {
                var checksumStart = BitConverter.ToUInt32(dataToFlash.Skip(0x101C4 + 8 * i).Take(4).Reverse().ToArray(), 0);
                var checksumEnd = BitConverter.ToUInt32(dataToFlash.Skip(0x101C8 + 8 * i).Take(4).Reverse().ToArray(), 0);
                ...
                {
                    checksumStart -= memSubtract;
                    checksumEnd -= memSubtract;
                }
            ...
            byte[] csCalcArray = BitConverter.GetBytes(crc).ToArray();
            for (int i = 0; i < 4; ++i)
                dataToFlash[maxAddress - memSubtract + 1 + i] = csCalcArray[3 - i];
        ...
        public static byte[] BypassRsa(byte[] binary)
        {
            /*Basically all we're doing is doing some binary patches.
             * Probably should verify that these work on all program variants, but I'm confident it's fine
             * Majority of people will be on newest programs anyway
             */

            byte[] rsaCheck = { 0x81, 0x86, 0x00, 0x00, 0x28, 0x0C, 0x00, 0x20, 0x40, 0x81, 0x00, 0x0C, 0x38, 0x60, 0xFF, 0xFF }; //At this point, the DME is checking if the signature is a valid length or not. 
            byte[] rsaPatch = { 0x81, 0x86, 0x00, 0x00, 0x28, 0x0C, 0x00, 0x00, 0x41, 0x80, 0x00, 0x0C, 0x38, 0x60, 0x00, 0x00 }; //The patch modifies the code to always think the length is incorrect, and return a 0 (i.e passed) instead of -1 (fail).

            byte[] isnReadProtect = { 0x48, 0x00, 0x00, 0x10, 0x38, 0x60, 0x00, 0x04 }; //Changes a return value from 04 to 03 (DME will send the buffer if 03, will send an FF or 00 if 04). 
            byte[] isnReadPatch =   { 0x48, 0x00, 0x00, 0x10, 0x38, 0x60, 0x00, 0x03 };

            int indexOfRsaSequence = SearchBytes(binary, rsaCheck);

            byte[] binaryPatched = binary;
            if (indexOfRsaSequence != -1)
            {
                for (int i = 0; i < rsaPatch.Length; ++i)
                    binaryPatched[indexOfRsaSequence + i] = rsaPatch[i];
            }
            ...
            int indexOfIsnProtectSequence = SearchBytes(binary, isnReadProtect);
            if (indexOfIsnProtectSequence != -1)
            {
                for (int i = 0; i < isnReadPatch.Length; ++i)
                    binaryPatched[indexOfIsnProtectSequence + i] = isnReadPatch[i];
            }
            ...
            for (int i = 0; i < 0x2C; ++i)
                binaryPatched[0x10204 + i] = binaryPatched[0x101c0 + i];
            ...
            for (int i = 0; i < 0xC0; ++i)
                binaryPatched[0x10040 + i] = 0xFF;

            return binaryPatched;
        }

        public static bool IsParamSignatureValid(byte[] binary)
        {
            byte[] signatureInjection = binary.Skip(0x104).Take(0x80).ToArray();
            byte[] calculatedMd5Injection = CheckParamMd5(binary.Take(0x10000).ToArray());
            byte[] decryptedMd5Injection = DecryptSignature(signatureInjection);
            bool injectionMatch = calculatedMd5Injection.SequenceEqual(decryptedMd5Injection);

            byte[] signatureIgnition = binary.Skip(0x10104).Take(0x80).ToArray();
            byte[] calculatedMd5Ignition = CheckParamMd5(binary.Skip(0x10000).Take(0x10000).ToArray());
            byte[] decryptedMd5Ignition = DecryptSignature(signatureIgnition);
            bool ignitionMatch = calculatedMd5Ignition.SequenceEqual(decryptedMd5Ignition);
            return injectionMatch && ignitionMatch;
        }

        public static bool IsProgramSignatureValid(byte[] binary)
        {
            byte[] signatureInjection = binary.Skip(0x10104).Take(0x80).ToArray();
            byte[] calculatedMd5Injection = CheckProgramMd5(binary.Take(0x280000).ToArray());
            byte[] decryptedMd5Injection = DecryptSignature(signatureInjection);
            bool injectionMatch = calculatedMd5Injection.SequenceEqual(decryptedMd5Injection);

            byte[] signatureIgnition = binary.Skip(0x290104).Take(0x80).ToArray();
            byte[] calculatedMd5Ignition = CheckProgramMd5(binary.Skip(0x280000).Take(0x280000).ToArray());
            byte[] decryptedMd5Ignition = DecryptSignature(signatureIgnition);
            bool ignitionMatch = calculatedMd5Ignition.SequenceEqual(decryptedMd5Ignition);
            return injectionMatch && ignitionMatch;
        }

        private static byte[] CheckParamMd5(byte[] binary)
        {
            uint numberOfSegments = BitConverter.ToUInt32(binary.Skip(0x1C0).Take(4).Reverse().ToArray(), 0);
            uint memSubtract = 0x70000;

            byte[] toHash = Array.Empty<byte>();
            for (int i = 0; i < numberOfSegments; ++i)
            {
                var segmentStart = BitConverter.ToUInt32(binary.Skip(0x1C4 + 8 * i).Take(4).Reverse().ToArray(), 0) - memSubtract;

                var segmentEnd = BitConverter.ToUInt32(binary.Skip(0x1C8 + 8 * i).Take(4).Reverse().ToArray(), 0) - memSubtract;

                toHash = toHash.Concat(binary.Skip((int)segmentStart).Take((int)(segmentEnd - segmentStart) + 1)).ToArray();
            }
        ...
        private static byte[] CheckProgramMd5(byte[] binary)
        {
            ...
            for (int i = 0; i < numberOfSegments; ++i)
            {
                var segmentStart = BitConverter.ToUInt32(binary.Skip(0x101C4 + 8 * i).Take(4).Reverse().ToArray(), 0);

                var segmentEnd = BitConverter.ToUInt32(binary.Skip(0x101C8 + 8 * i).Take(4).Reverse().ToArray(), 0);

                if (segmentStart >= 0x450000)
                {
                    segmentStart -= memSubtract;
                    segmentEnd -= memSubtract;
                }
        ...
        private static byte[] DecryptSignature(byte[] signature)
        {
            var n = BigInteger.Parse("");
            ...
            byte[] signatureReverseDword = Array.Empty<byte>();
        ...
        private static uint Crc32(byte[] buffer, uint initial)
        {
            uint[] table =
            {
            ...
            {
                return buffer.Aggregate(initial, (current, t) => table[(current ^ t) & 0xFF] ^ (current >> 8));
            }
        }
